"""
export_bytetracker.py — Exporta un vídeo de tracking con BYTETracker
El vídeo se guarda en la carpeta videos/ en la raíz del proyecto.

Uso:
    python src/tracking/export/export_bytetracker.py
"""
...
output_path = os.path.join(OUTPUT_DIR, f"{SEQUENCE}_bytetracker.mp4")
...
print(f"Exportando {SEQUENCE} con BYTETracker...")
...
for i, img_name in enumerate(images, start=1):
    ...
    # ── Info en pantalla ──
    cv2.putText(frame, f"BYTETracker | {SEQUENCE} | Frame {i}/{len(images)}",
