...
results_dir = os.path.join(os.path.dirname(__file__), '..', '..', 'results', 'ocsort')
...
for i, img_name in enumerate(images, start=1):
    ...
    for (x, y, w, h, conf) in dets_frame:
        ...
        if area < BALL_MAX_AREA and BALL_MIN_RATIO < ratio < BALL_MAX_RATIO:
            ...
            # OCSORT espera [x1, y1, x2, y2, confianza, clase]
    ...
    cv2.putText(frame, "Algoritmo: OCSORT",
                (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)

    cv2.imshow("OCSORT Tracking — SoccerNet", frame)
