...
Esta discrepancia no es un error de cálculo ni una casualidad estadística:
tiene una explicación mecánica concreta, visible en las columnas de
...
detecciones originales (previsiblemente por la interpolación de posición
...
calculada en un único umbral de IoU relativamente permisivo (0,5), pero se
...
original}, y es esta diferencia de precisión geométrica, no de robustez de
asociación, la que explica por qué la métrica más exigente (HOTA integrada)
favorece a OC-SORT mientras que la métrica de umbral único (HOTA(0))
favorece a ByteTrack.

La conclusión que se extrae de este contraste, y que corrige la lectura
...
métrica íntegra y más robusta del campo. Dicho esto, ByteTrack no queda
descartado sin matices: su menor precisión de localización es
...
El estudio de hiperparámetros se reporta en términos de HOTA(0) y no de HOTA integrada
. La
Figura~\ref{fig:pruebas-hiperparametros} muestra el efecto de los dos
hiperparámetros con mayor impacto observado, uno por algoritmo.
...
El resultado más relevante de este estudio, ya adelantado en la
sección~\ref{sec:implementacion-hiperparametros}, es que \textbf{el ajuste
...
\begin{itemize}
  ...
  \item \textbf{Posesión como heurística, no como estadística oficial}: los
  ...
  \item \textbf{Detección del balón por criterio geométrico}: cualquier
        resultado que dependa de la localización del balón hereda las limitaciones del filtro geométrico frente a
        un detector entrenado.
\end{itemize}
