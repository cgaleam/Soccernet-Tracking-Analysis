...
\node[box, fill=gray!10, minimum width=3.4cm] (soccernet) {SoccerNet-Tracking};
\node[box, fill=blue!10, minimum width=4.6cm, right=1.6cm of soccernet] (pipeline) {Pipeline(Python)};
\node[box, fill=orange!12, minimum width=4.2cm, right=1.6cm of pipeline] (web) {Aplicación web};
\draw[arrow] (soccernet) -- (pipeline) node[midway, above, font=\footnotesize] {\texttt{det.txt}, \texttt{gt.txt}};
\draw[arrow] (pipeline) -- (web) node[midway, above, font=\footnotesize] {resultados exportados};
\node[below=0.5cm of pipeline, font=\footnotesize, align=center, text width=8cm]
  {El pipeline consume el dataset y produce resultados persistidos en disco
   (CSV, PNG, MP4); la aplicación web los consume ya calculados,
   sin ejecutar ninguna fase del pipeline.};
\end{tikzpicture}
...
proyecto, se exploraron brevemente otras tareas del ecosistema SoccerNet
(sección~\ref{sec:fundamentos-dataset}), como parte de la primera fase del
...
el dataset SoccerNet-Tracking incluye detecciones ya calculadas
(\texttt{det.txt}, sección~\ref{subsec:fundamentos-soccernet-tracking}), lo
...
pasar de cero a visualizaciones funcionales sobre el \emph{ground truth}
(scripts descritos en la sección~\ref{sec:implementacion-tracking}); el
...
necesitaba. Para la evaluación se adoptó \texttt{TrackEval}
(sección~\ref{sec:implementacion-evaluacion}), por ser la herramienta de
