...
Este capítulo presenta, de lo general a lo particular, todos los conceptos
teóricos necesarios para comprender el resto de este proyecto. Se parte de
las nociones más básicas: qué es la inteligencia artificial, qué es un
dataset y por qué existe SoccerNet. Y se profundiza progresivamente hasta
el detalle técnico de cada algoritmo, métrica y técnica empleados: el
problema del seguimiento multiobjeto, los algoritmos de \emph{tracking-by-detection}
comparados (ByteTrack y OC-SORT) y los que se consideraron pero no llegaron
a implementarse, las métricas de evaluación estándar del campo (MOTA, IDF1,
HOTA), la clasificación de jugadores por color mediante K-means, y los
fundamentos de la homografía aplicada a la proyección de coordenadas.
Las decisiones concretas de este proyecto como parámetros, configuración,
qué resultado se obtuvo no se abordan aquí, sino en los capítulos de
...
directamente representaciones útiles de los datos en bruto --p.\ ej.,
píxeles de una imagen-- sin necesidad de que un experto humano diseñe a
...
(p.\ ej., K-means, sección~\ref{sec:fundamentos-kmeans}).}
...
\begin{enumerate}
  \item \textbf{Detección}: localizar dónde está cada objeto de interés en
        ...
        aprendizaje profundo (p.\ ej., la familia YOLO, discutida en la
...
(sección~\ref{sec:intro-problema}).}
...
(p.\ ej., IoU mínima) por debajo del cual una predicción y una detección no
...
(sección~\ref{subsec:fundamentos-bytetrack}).
...
Kalman a los movimientos no lineales (sección~\ref{sec:fundamentos-mot}),
...
Puesto en perspectiva, el HOTA integrado en test obtenido en este proyecto
con OC-SORT (75{,}43), resulta muy próximo al del equipo ganador del
reto oficial 2023 (75{,}61) pese a partir de una posición menos favorable
...
tanto, de una comparación estrictamente equivalente ni de una pretensión
de superar el estado del arte del reto, sino de una referencia
...
La aportación de este proyecto no busca, en consecuencia, competir por la
mejor posición en ese \emph{leaderboard}, sino dos objetivos distintos que
el propio reto no cubre: primero, una comparación controlada y
metodológicamente rigurosa entre dos algoritmos clásicos de
\emph{tracking-by-detection} bajo condiciones idénticas: mismas
detecciones, mismas métricas, mismo estudio sistemático de
hiperparámetros, que documenta explícitamente una discrepancia
metodológica entre HOTA y HOTA(0) no discutida en los informes de
resultados del propio reto; y segundo, la construcción de un pipeline
completo de extracción de metadatos deportivos interpretables y su
exposición en una aplicación web, que va
