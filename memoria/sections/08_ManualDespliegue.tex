...
conocida y ya documentada: \texttt{trackeval} no aparece listado explícitamente en
...
como dependencia transitiva de otro paquete, instalarlo aparte con
\texttt{pip install trackeval}.

\subsection{Descarga del dataset SoccerNet}
\label{subsec:manual-dataset}

La descarga se automatiza con el SDK oficial de SoccerNet, ya incluido en
...
\texttt{SoccerNet/} creada como hermana del repositorio, reproduciendo la estructura
...
(sección~\ref{subsec:diseno-web-ui}). \textbf{Comparativa de algoritmos}
muestra, sin necesidad de seleccionar nada, la tabla de métricas
HOTA/HOTA(0)/DetA/AssA/MOTA/IDF1 por \emph{split}, la configuración óptima
de cada algoritmo y las gráficas del estudio de hiperparámetros.
\textbf{Explorador de secuencias} requiere elegir una secuencia y un algoritmo mediante los
selectores de chips; el resto de la pestaña se
actualiza automáticamente con los datos de esa combinación, mostrando una
nota de \enquote{no disponible} en los paneles sin datos.

Las Figuras~\ref{fig:manual-web-comparativa} a~\ref{fig:manual-web-118}
son capturas reales de la aplicación en ejecución (\texttt{npm run dev})
sobre el paquete de datos descrito en la
sección~\ref{subsec:impl-web-bundle}.

\begin{figure}[htp]
\centering
\includegraphics[height=0.8\textheight, keepaspectratio]{figures/manual_web_comparativa.png}
\caption{Pestaña \enquote{Comparativa de algoritmos}.}
...
\caption{Pestaña \enquote{Explorador de secuencias} con SNMOT-116 y ByteTrack.}
