...
\begin{tabular}{|p{2.6cm}|p{1.8cm}|p{7.0cm}|}
\hline
\rowcolor{gray!25}
...
\texttt{trackeval} & \texttt{sn-trackeval} 0.4.0 & Evaluación con
  ...
  sección~\ref{sec:fundamentos-metricas}). Fork de TrackEval adaptado por
...
proyecto (sección~\ref{sec:gestion-fases}) se dedicó a comprender el
...
El nombre de este último fichero, \texttt{sort\_alg.py}, es una muestra
histórica, documentada en la sección~\ref{subsec:fundamentos-sort}: la
intención original era implementar el SORT clásico, pero \texttt{boxmot}
en su versión fijada (P4) no expone una clase \texttt{SORT} independiente,
solo \texttt{OCSORT}, su evolución directa. El fichero conserva su nombre
...
La evaluación (diseñada en la sección~\ref{sec:diseno-evaluacion}) se
...
Los resultados de este barrido se resumen en la Tabla~\ref{tab:impl-hiperparametros-resultados}
para los hiperparámetros con efecto observable. Los hiperparámetros
\texttt{track\_thresh} (ByteTrack) y \texttt{det\_thresh} (OC-SORT) no
mostraron ningún efecto sobre la métrica en el rango probado, y se omiten
de la tabla por brevedad.

\begin{table}[htp]
\centering
\caption{Efecto de cada hiperparámetro sobre HOTA(0) (\%) en el conjunto de test.}
\label{tab:impl-hiperparametros-resultados}
...
A partir de estos resultados individuales, \texttt{bytetracker\_optimal.py}
y \texttt{ocsort\_optimal.py} fijan y validan la configuración combinada
que RF-008 exige confirmar de forma independiente:
...
(manteniendo \texttt{det\_thresh=0.3}, sin efecto). Al ejecutar y evaluar
estas configuraciones combinadas de forma independiente, se obtiene HOTA(0)
= 81{,}43 para ByteTrack y HOTA(0) = 80{,}92 para OC-SORT.
...
conjunto. En
OC-SORT, en cambio, la combinación de los tres hiperparámetros (80{,}92)
mejora claramente sobre cualquiera de ellos por separado (mejor individual:
80{,}17 con \texttt{iou\_threshold=0.1}), lo que indica que en este caso sí
se refuerzan entre sí. Esta configuración óptima de ByteTrack, por ser la
de mayor HOTA(0) absoluto entre ambos algoritmos, es la que se usa como
base de todo el pipeline de metadatos de la
...
se calcula la distancia total recorrida (suma de las distancias euclídeas
entre posiciones consecutivas) y la velocidad, tanto media como de pico
--esta última como el percentil 95 de las velocidades instantáneas
calculadas sobre una ventana de 1 segundo.

Para los mapas de calor calibrados con esta homografía real
(\texttt{heatmap\_homography\_demo\_116.py} y
\texttt{heatmap\_homography\_demo\_118.py}), el diseño incorpora además el
sombreado de honestidad visual descrito en la
...
señal de que su trayectoria de tracking se desvía significativamente de la
zona de cámara estable para la que se calculó esa homografía concreta.
